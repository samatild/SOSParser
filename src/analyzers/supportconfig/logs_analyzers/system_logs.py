...
class SystemLogsAnalyzer:
    ...
    def analyze(self) -> Dict[str, Any]:
        """Analyze system logs."""
        system_logs = {}
        messages = self.parser.read_file_content('messages.txt')
        if messages:
            system_logs['messages'] = messages
        messages_config = self.parser.read_file_content('messages_config.txt')
        if messages_config:
            system_logs['syslog'] = messages_config
        messages_localwarn = self.parser.read_file_content('messages_localwarn.txt')
