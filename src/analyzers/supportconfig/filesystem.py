...
class SupportconfigFilesystem:
    ...
    def analyze(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'filesystem_types': self.get_filesystem_types(),
        }
    ...
    def get_filesystem_types(self) -> Dict[str, Any]:
        ...
        # Get btrfs info
        content = self.parser.read_file('fs-btrfs.txt')
        if content:
            fs_types['btrfs'] = content[:2000]  # First 2000 chars
        
        # Get autofs info
        autofs = self.parser.read_file('fs-autofs.txt')
        if autofs and len(autofs) > 100:  # Check if not just header
            fs_types['autofs'] = autofs[:1000]
